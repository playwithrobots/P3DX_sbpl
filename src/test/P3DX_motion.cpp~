...
#define SE 7
#define SW 8
#define DISTANCE_H 400		// width of each cell 40cm 
#define DISTANCE_D (DISTANCE_H * 1.4 )
#define SONAR_NUM 8
#define SONAR_THRESH (560)   // sonar threshhold 56 cm 
unsigned char P3DX_dir=4;	// initial robot direction 
int x,y,newx,newy; 
...
// turn on motors, and on sonar, and on amigobot sounds, when connected
...
// based on intial and final coordinates it calculates angle and fwd distance to move 
void go_P3DX(void)
...
void start_fun(const P3DX_sbpl::sbpl_msg::ConstPtr& msg)
{    
...
sbpl_wait_pub = n.advertise<P3DX_sbpl::sbpl_msg>("sbpl_wait",10);
sonar_pub = n.advertise<P3DX_sbpl::sonar_msg>("sonar",10);
ros::Subscriber start_sub = n.subscribe("sbpl_xy", 10, start_fun); 
  ArArgumentParser argParser(&argc, argv);
  ArSimpleConnector con(&argParser);
  ArRobot robot;
  P3DX_sbpl::sbpl_msg msg;
  P3DX_sbpl::sonar_msg sonar; 
...
{  	if(start_flag)
		{ 	
			go_P3DX(); // calculate distance and angle
			ROS_INFO(" distance = %d angle=%d ",result[0],result[1]);
			robot.lock();
			robot.setDeltaHeading(result[1]); // rotate robot
			robot.unlock();
			start.setToNow();
			while (1)
			  {	    robot.lock();
			 ...
			 robot.lock();
			 robot.move(result[0]);// move robot
			 robot.unlock();
			 start.setToNow();
			 while (1)
			 {
